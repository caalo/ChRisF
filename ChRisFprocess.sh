...
do   
	...
	if [ $firstRun -eq 1 ]; then
		./ChRisF_learner -mode bandit -loadmodel none -trainf $TRAINFILE -lrate $l -lambda 0 -nsentence all -epoch $EPOCHS_PER_DECODE -loss $loss -savemodel $fname 
		firstRun=0
	else
		./ChRisF_learner -mode bandit -loadmodel $prevModel -trainf $TRAINFILE -lrate $l -lambda 0 -nsentence all -epoch $EPOCHS_PER_DECODE -loss $loss -savemodel $fname
	fi
