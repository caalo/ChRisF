...
#second arg is loss function: bayes, crossentropy, pair, probit
l=$1
loss="fullinfo"

FOLDER="models$loss$l/"
DATE=`date +%Y-%m-%d:%H:%M`
...
do   
	...
	if [ $firstRun -eq 1 ]; then
		./ChRisF_learner -mode fullinfo -loadmodel none -trainf $TRAINFILE -lrate $l -lambda 0 -nsentence all -epoch $EPOCHS_PER_DECODE -loss $loss -savemodel $fname 2> $fname2
		firstRun=0
	else
		./ChRisF_learner -mode fullinfo -loadmodel $prevModel -trainf $TRAINFILE -lrate $l -lambda 0 -nsentence all -epoch $EPOCHS_PER_DECODE -loss $loss -savemodel $fname 2> $fname2
	fi
		
	#decode on dev, append to text file
	f1=$FOLDER
	f1+="epoch"
	f1+=$epoch
	f1+="_decode_dev.txt"
